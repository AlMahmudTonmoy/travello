...
(function($) {
	...
	MultiSelect.prototype = {
		...
		load: function() {
			...
			$(document)
				...
				.on('click.ms-hideopts', function(event) {
					if (!$(event.target)
						...
						.length) {
						$('.ms-options-wrap > .ms-options:visible')
							.hide();
					}
				});
				...
				search.on('keyup', function() {
					...
					optionsList.find('li:not(.optgroup)')
						.each(function() {
							...
							if (!instance.options.searchOptions.showOptGroups && $(this)
								.closest('li.optgroup')) {
								$(this)
									.closest('li.optgroup')
									.show();
			...
			optionsWrap.on('click', '.ms-selectall', function(event) {
				...
				if ($(this)
					...
					if (optionsList.find('li:not(.optgroup)')
						.filter(':not(.selected)')
						.length) {
						optionsList.find('li:not(.optgroup)')
							.filter(':not(.selected)')
					...
					// deselect everything
						...
						optionsList.find('li:not(.optgroup).selected input[type="checkbox"]')
							.trigger('click');
					}
				} else if ($(this)
					.closest('li')
					...
					// check if any selected if so then select them
					if (optgroup.find('li:not(.selected)')
						.length) {
						optgroup.find('li:not(.selected) input[type="checkbox"]')
							.trigger('click');
					}
					// deselect everything
					else {
		...
		loadOptions: function(options, overwrite) {
			...
			optionsList.find('.ms-reflow input[type="checkbox"]')
				.each(function(idx) {
					if ($(this)
						.css('display')
						.match(/block$/)) {
						var checkboxWidth = $(this)
							.outerWidth();
		...
		_addOption: function(container, option) {
			...
			if (!this.options.showCheckbox) {
				container.find('input[id="ms-opt-' + msCounter + '"]')
					.hide();
			}
